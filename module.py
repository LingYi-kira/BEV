...
class AverageKeyMeter(MeanMetric):
    '''Calculate the average of the values corresponding 
        to specific keys in the dictionary.
    '''
...
class GenericModule(pl.LightningModule):
    ...
    def pred_bev(self, batch):
        image = batch["image"][:, 0, :, :].squeeze(dim=1)
        camera = batch["camera"]
        device = batch["image"].device
        
        return self.model.imageTobev(image, camera, device)
    ...
    def training_step(self, batch):
        ...
        # raise RuntimeError("Stop after printing sizes")  # for test

        return losses["total_loss"].mean()
    ...
    def load_from_checkpoint(
        ...
        logger.info(
            "Using checkpoint %s from epoch %d.",
